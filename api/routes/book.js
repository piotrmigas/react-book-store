...
const getPredicate =
  ({ title = '', author = '' }) =>
  (item) =>
    item.title.toLowerCase().match(title.toLowerCase()) && item.author.toLowerCase().match(author.toLowerCase());
...
router.get('/', (req, res, next) => {
  ...
  const result = filteredBooks.slice((page - 1) * limit, page * limit).map((book) => getBookWithCoverUrl(req, book));
